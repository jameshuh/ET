...
System.namespace("DCET.Hotfix", function (namespace)
  namespace.class("AnimatorComponentAwakeSystem", function (namespace)
    ...
    return {
      base = function (out)
        return {
          out.DCET.Hotfix.AwakeSystem_1(out.DCET.Hotfix.AnimatorComponent)
        }
      end,
      Awake = Awake,
  ...
  namespace.class("AnimatorComponentUpdateSystem", function (namespace)
    ...
    return {
      base = function (out)
        return {
          out.DCET.Hotfix.UpdateSystem_1(out.DCET.Hotfix.AnimatorComponent)
        }
      end,
      Update = Update,
      __metadata__ = function (out)
  ...
  namespace.class("AnimatorComponent", function (namespace)
    ...
    Awake = function (this)
      local animator = this:GetParent(DCETHotfix.Unit).ViewGO:GetComponent(UnityEngine.Animator)

      if UnityEngine.op_Equality(animator, nil) then
        return
      end

      if UnityEngine.op_Equality(animator:getruntimeAnimatorController(), nil) then
        return
      end
    ...
    Update = function (this)
      ...
      if this.MotionType == 0 --[[MotionType.None]] then
        return
      end
      ...
      System.try(function ()
        ...
        this.Animator:SetTrigger(this.MotionType:EnumToString(DCETHotfix.MotionType))

        this.MontionSpeed = 1
        this.MotionType = 0 --[[MotionType.None]]
      end, function (default)
        local ex = default
        System.throw(System.Exception("动作播放失败: " .. this.MotionType:EnumToString(DCETHotfix.MotionType), ex))
      end)
    end
    ...
    PlayInTime = function (this, motionType, time)
      local animationClip
      local default
      default, animationClip = this.animationClips:TryGetValue(motionType:EnumToString(DCETHotfix.MotionType))
      if not default then
        System.throw(System.Exception("找不到该动作: " .. motionType:EnumToString(DCETHotfix.MotionType)))
      end

      local motionSpeed = animationClip:getlength() / time
      if motionSpeed < 0.01 or motionSpeed > 1000 then
        DCETHotfix.Log.Error1("motionSpeed数值异常, " .. motionSpeed .. ", 此动作跳过")
        return
      end
    ...
    Play = function (this, motionType, motionSpeed)
      if not HasParameter(this, motionType:EnumToString(DCETHotfix.MotionType)) then
        return
      end
      this.MotionType = motionType
      this.MontionSpeed = motionSpeed
    ...
    PauseAnimator = function (this)
      ...
      if UnityEngine.op_Equality(this.Animator, nil) then
        return
      end
