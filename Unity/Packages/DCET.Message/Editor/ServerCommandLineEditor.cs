...
namespace DCETEditor
{
	...
	public class StartConfigDrawer: Entity
	{
		...
		public bool OnGUI()
		{
...

			{
				...
				{
					Assembly assembly = Assembly.GetAssembly(typeof(ConfigComponent));
					Type type = assembly.GetType($"DCET.{st}");
					startConfig.AddComponentWithType(type);
				}
				...
				if (GUILayout.Button("删除组件", GUILayout.Height(16)))
				{
					Assembly assembly = Assembly.GetAssembly(typeof(ConfigComponent));
					Type type = assembly.GetType($"DCET.{st}");
					startConfig.RemoveComponentWithType(type);
				}
	...
	public class ServerCommandLineEditor: EditorWindow
	{
		...
		private void OnEnable()
		{
			...
			Game.EventSystem.AddRangeType(typeof(ServerCommandLineEditor).Assembly.GetTypes());
