...
namespace ProtoBuf.Serializers
{
	internal class ListDecorator : ProtoDecoratorBase
	{
		...
		protected MethodInfo GetEnumeratorInfo(TypeModel model, out MethodInfo moveNext, out MethodInfo current)
		{
			...
			Type expectedType2 = base.Tail.ExpectedType;
			Type returnType;
			if (instanceMethod != (MethodInfo)null)
			{
				...
				current = ((property == (PropertyInfo)null) ? null : Helpers.GetGetMethod(property, false, false));
				if (moveNext == (MethodInfo)null && model.MapType(ienumeratorType).IsAssignableFrom(returnType))
				{
			...
			if (type != (Type)null && type.IsAssignableFrom(expectedType))
			{
