...
- Entity
  ...
  public class SkillChargeComponent : Entity
  {
      ...
      {
          // 蓄力结束
          // 发出蓄力结束事件，Handler后释放技能
      }
  }
  ```

  根据以上示例可以看出，当为蓄力型技能时，可以为Skill加上SkillChargeComponent组件，技能释放时就可以走蓄力逻辑；当技能为直接释放型技能时，则不需要加SkillChargeComponent组件，也就不会走蓄力逻辑，这样一来则可以轻松重用代码逻辑，并且代码组织结构也十分清晰。
